...
### Added
- hadling of wrong body field name for svg and file uploads

### Removed
- file size checks on uploads (must be declared in nginx config, see [example](./nginx_example_configs/conf.d/otp/otp_rest.conf))

### Fixed
- decoding of files in user activity logging and writing their content to logs
...
## [1.18.0] - 2023-07-11
### Added
- Endpoint `api/load/file` for uploading static files

### Changed
- SvgLoadHandler to FileLoadHandler in order to reuse the same class for both endpoints (svg,static)
